...
// class AABB——轴向包围盒构成的层次包围盒树的节点类

namespace igl
{
    /*
       Implementation of semi-general purpose axis-aligned bounding box hierarchy.
       The mesh (vers, Ele) is stored and managed by the caller and each routine here
       ...
       picked up at run time from vers. This leads to duplicated code for 2d/3d (up to
       dim).
    */
...
public:
      ...
      // 成员数据：
      AABB* m_left;                 // 左孩子节点，Shared pointers are slower...
      AABB* m_right;
      Eigen::AlignedBox<Scalar,DIM> m_box;      // 轴向包围盒对象；
      int m_primitive;              // -1 non-leaf
      ...
      // Seems like there should have been an elegant solution to this using the copy-swap idiom above:
      ...
      */ 
      ...
      //
      // Inputs:
      //   vers  #vers by dim list of mesh vertex positions. **Should be same as used to
      //     construct mesh.**
      //   Ele  #Ele by dim+1 list of mesh indices into #vers. **Should be same as used to
      //     construct mesh.**
      ...
      /*
           ...
           Inputs:
             ...
             Ele        #Ele by dim list of simplex indices
             ver0          dim-long query point 
             low_sqr_d       lower bound on squared distance, specified maximum squared  distance 
             up_sqr_d        current upper bounded on squared distance, current minimum squared distance (
                                            only consider distances less than this), see output.
      ...
      */
      ...
      // squared_distance()重载1.1——Default low_sqr_d
      template <typename DerivedEle>
      IGL_INLINE Scalar squared_distance(
        const Eigen::MatrixBase<DerivedV> & vers,
        const Eigen::MatrixBase<DerivedEle> & Ele, 
...
public:
      // Compute the squared distance from all query points in vers0 to the
      ...
      //   vers  #vers by dim list of vertex positions
      //   Ele  #Ele by dim list of simplex indices
      //   vers0  #vers0 by dim list of query points
      // Outputs:
      //   minSqrDis  #vers0 list of squared distances
      //   I  #vers0 list of indices into Ele of closest primitives
      //   C  #vers0 by dim list of closest points
      template <
        typename DerivedEle,
        typename DerivedP, 
      ...
      // squared_distance()重载2——两个BVH的比较
      /*
           Compute the squared distance from all query points in vers0 already stored
           in its own AABB hierarchy to the _closest_ points on the primitives
           stored in the AABB hierarchy for the mesh (vers,Ele).
...
private:
      ...
      */
      ...
      //
      // Inputs:
      //   ver0  dim-long query point (only used in DEBUG mode)
