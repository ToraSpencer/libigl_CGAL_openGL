...
namespace igl
{
  ...
  // set operations.  **Assumes** that `(vers,tris)` contains no self-intersections
  ...
  // resolve all self-intersections in `(vers,tris) -> (SV,SF)` (i.e. what the
  // `igl::copyleft::cgal::piecewise_constant_winding_number` overload does).
  //
  // Inputs:
  //   tris  #tris by 3 list of triangle indices into some (abstract) list of
  //     vertices vers
  //   uE  #uE by 2 list of unique edges indices into vers
  //   uE2E  #uE list of lists of indices into directed edges (#tris * 3)
