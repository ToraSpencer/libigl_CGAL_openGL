...
namespace igl
{
    // 重载1： 测量两个网格的hausdorff距离；
    /*
       HAUSDORFF compute the Hausdorff distance between mesh (VA,FA) and mesh (VB,FB). 

       d(A,B) = max ( max min d(a,b) , max min d(b,a) )
                      a∈A b∈B          b∈B a∈A
  
       Known issue: This is only computing max(min(va,B),min(vb,A)). 
             This is better than max(min(va,Vb),min(vb,Va)). 
       
       This (at least) is missing "edge-edge" cases like the distance between the two different
                  triangulations of a non-planar quad in 3D.
       
       Even simpler, consider the Hausdorff distance between the non-convex, block letter vers polygon (with 7
               vertices) in 2D and its convex hull. 
       The Hausdorff distance is defined by the midpoint in the middle of the segment across the concavity and some
                non-vertex point _on the edge_ of the vers.

       Known issue: due to the issue above, this also means that unreferenced vertices can give unexpected results. 
            Therefore, we assume the inputs have no unreferenced vertices.
