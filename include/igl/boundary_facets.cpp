  ...
  Eigen::PlainObjectBase<DerivedK>& K)
{
  ...
  tris.resize(ones, allF.cols());
  J.resize(tris.rows(),1);
  K.resize(tris.rows(),1);
  int k = 0;
  for(int c = 0;c< (int)C.size();c++)
  {
    ...
    {
      const int i = m(c);
      assert(k<(int)tris.rows());
      tris.row(k) = allF.row(i);
      J(k) = i/simplex_size;
      K(k) = i%simplex_size;
      k++;
    }
...
{
  Eigen::VectorXi J,K;
  return boundary_facets(T,tris,J,K);
}
