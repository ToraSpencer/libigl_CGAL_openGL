  ...
  std::vector< std::tuple<Eigen::MatrixXd, Eigen::RowVectorXd, double> > & quadrics)
{
  ...
  quadrics.resize(vers.rows(),  Quadric{Eigen::MatrixXd::Zero(dim, dim), Eigen::RowVectorXd::Zero(dim), 0});     // gcc <=4.8 can't handle initializer lists correctly
  ...
  for(int v = 0; v<vers.rows(); v++)
  {
    ...
    Eigen::RowVectorXd Vv = vers.row(v); 
  ...
  for(int f = 0; f<tris.rows(); f++)
  {
    ...
    {
      if(
         std::isinf(vers(tris(f, c), 0)) || 
         std::isinf(vers(tris(f, c), 1)) || 
         std::isinf(vers(tris(f, c), 2)))
      {
    ...
    */
    const auto subspace_quadric = [&I](const Eigen::RowVectorXd & p, const Eigen::MatrixXd & S,  const double  weight)->Quadric
    {
      ...
      // gcc <=4.8 can't handle initializer lists correctly: needs explicit cast
    ...
    if(infinite_corner == -1)
    {
      ...
      Eigen::RowVectorXd p = vers.row(tris(f, 0)); 
      Eigen::RowVectorXd q = vers.row(tris(f, 1)); 
      Eigen::RowVectorXd r = vers.row(tris(f, 2)); 
      ...
      Eigen::MatrixXd S(2, vers.cols()); 
      S<<e1, e2; 
      Quadric face_quadric = subspace_quadric(p, S, area); 
      // Throw at each corner
      for(int c = 0; c<3; c++)
      {
        quadrics[tris(f, c)] = quadrics[tris(f, c)] + face_quadric; 
      }
    }
    else
    {
      ...
      const Eigen::RowVectorXd p = vers.row(tris(f, (infinite_corner+1)%3)); 
      Eigen::RowVectorXd ev = vers.row(tris(f, (infinite_corner+2)%3)) - p; 
      const double length = ev.norm(); 
      ev /= length; 
      // Face neighbor across boundary edge
      int e = edgeUeInfo(f+tris.rows()*infinite_corner); 
      int opp = UeTrisInfo(e, 0) == f ? 1 : 0; 
      int n =  UeTrisInfo(e, opp); 
      int nc = EI(e, opp); 
      assert(
        ((tris(f, (infinite_corner+1)%3) == tris(n, (nc+1)%3) && 
          tris(f, (infinite_corner+2)%3) == tris(n, (nc+2)%3)) || 
          (tris(f, (infinite_corner+1)%3) == tris(n, (nc+2)%3) 
          && tris(f, (infinite_corner+2)%3) == tris(n, (nc+1)%3))) && 
        "Edge flaps not agreeing on shared edge"); 
      // Edge vector on opposite face
      const Eigen::RowVectorXd eu = vers.row(tris(n, nc)) - p; 
      ...
      for(int c = 0; c<3; c++)
      {
        ...
        {
          quadrics[tris(f, c)] = quadrics[tris(f, c)] + boundary_edge_quadric; 
        }
      }
    }
