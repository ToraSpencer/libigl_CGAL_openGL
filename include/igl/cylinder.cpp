...
{
  vers.resize(axis_devisions*height_devisions,3);
  tris.resize(2*(axis_devisions*(height_devisions-1)),3);
  int f = 0;
  ...
  for(int th = 0;th<axis_devisions;th++)
  {
    ...
    for(int h = 0;h<height_devisions;h++)
    {
      ...
      vers(th+h*axis_devisions,0) = x;
      vers(th+h*axis_devisions,1) = y;
      vers(th+h*axis_devisions,2) = z;
      if(h > 0)
      {
        tris(f,0) = ((th+0)%axis_devisions)+(h-1)*axis_devisions;
        tris(f,1) = ((th+1)%axis_devisions)+(h-1)*axis_devisions;
        tris(f,2) = ((th+0)%axis_devisions)+(h+0)*axis_devisions;
        f++;
        tris(f,0) = ((th+1)%axis_devisions)+(h-1)*axis_devisions;
        tris(f,1) = ((th+1)%axis_devisions)+(h+0)*axis_devisions;
        tris(f,2) = ((th+0)%axis_devisions)+(h+0)*axis_devisions;
        f++;
      }
    }
